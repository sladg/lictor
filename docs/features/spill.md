...
Both streams are guarded independently: compilers and test runners emit their volume on **stderr**, so a bare `cargo build` spills there too (key suffix `-stderr`), not just when `2>&1` merges it into stdout.
